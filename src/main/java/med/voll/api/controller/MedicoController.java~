...
import med.voll.api.dto.medico.DadosDetalhamentoMedicos;
...
public class MedicoController {
    ...
    public ResponseEntity cadastrarMedico (@RequestBody @Valid DadosCadastroMedico dados, UriComponentsBuilder uriBuilder){
        ...
        var uri = uriBuilder.path("/medicos/{id}").buildAndExpand(medico.getId()).toUri(); //mostra o endpoint get com id no header
...
//        repository.deleteById(id); -> deletar em definitivo
        ...
        medico.inativar();
        return ResponseEntity.noContent().build(); // retornar 204 na api
